...
class Sandbox(Sketch):
    ...
    def draw(self):
        ...
        print("step", step, "resample count", resamp_count, "speed", speed)

        # create base shape
        rect = Rect((self.w - size) / 2, (self.h - size) / 2, size, size)

        # calculate drunk walk
        poly = rect.resample(num=resamp_count)
        ...
        if self.option_a:
            ...
            self.path(rect.vertices(), closed=True)
